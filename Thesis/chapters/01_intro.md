...
Despite their apparent simplicity, the thermal behaviour of shell boilers is governed by tightly coupled processes: multi-stage radiative and convective heat transfer, natural-circulation boiling inside the pressure parts, complex flue-gas property variations, and geometry-dependent hydraulic losses. Modern operating demands—higher efficiency, reduced emissions, increased reliability, and improved control—require predictive tools capable of resolving these coupled mechanisms across the full gas path and water/steam circuit.
...
representing the furnace, reversal chambers, convective tube banks, and the economiser. On the water side, the boiler drum provides a saturated interface for nucleate boiling in the pressure parts, while the economiser section is treated as a single-phase internal flow. Gas-side properties are supplied by Cantera, enabling temperature-dependent transport, specific heat, thermal conductivity, and radiative behaviour of $CO_2$–$H_2O$ mixtures.
...
4. To compute boiler-level performance, including useful heat transfer $Q_\text{useful}$, direct and indirect efficiencies, stack temperature, and stage-wise duties.

5. To evaluate sensitivity of boiler performance to key operating parameters—excess air ratio $\lambda$, feedwater/drum pressure, and fuel mass-flow rate.
