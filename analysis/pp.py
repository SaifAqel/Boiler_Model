"""
Sensitivity plotting script for boiler thesis

Reads:
    results/summary/boiler_kpis_all_runs.csv
    results/summary/stages_summary_all_runs.csv

Writes figures to:
    results/plots/sens

Required Python packages:
    pandas
    matplotlib
    numpy
"""

from pathlib import Path

import numpy as np
import pandas as pd
import matplotlib.pyplot as plt


# ---------------------------------------------------------------------------
# Paths and global setup
# ---------------------------------------------------------------------------

BASE_DIR = Path(".")
KPI_CSV = BASE_DIR / "results" / "summary" / "boiler_kpis_all_runs.csv"
STAGE_CSV = BASE_DIR / "results" / "summary" / "stages_summary_all_runs.csv"
PLOT_DIR = BASE_DIR / "results" / "plots" / "sens"

PLOT_DIR.mkdir(parents=True, exist_ok=True)

STAGE_ORDER = ["HX_1", "HX_2", "HX_3", "HX_4", "HX_5", "HX_6"]
STAGE_INDEX = {s: i for i, s in enumerate(STAGE_ORDER, start=1)}

plt.rcParams.update(
    {
        "figure.dpi": 150,
        "savefig.dpi": 300,
        "font.size": 9,
        "axes.grid": True,
        "grid.alpha": 0.3,
        "legend.frameon": False,
    }
)


# ---------------------------------------------------------------------------
# Utilities
# ---------------------------------------------------------------------------

def load_data():
    df_kpi = pd.read_csv(KPI_CSV)
    df_stage = pd.read_csv(STAGE_CSV)

    # Convert param_value to numeric where possible
    df_kpi["param_value"] = pd.to_numeric(df_kpi["param_value"], errors="coerce")
    df_stage["param_value"] = pd.to_numeric(df_stage["param_value"], errors="coerce")

    # Sort for nice plotting
    df_kpi = df_kpi.sort_values(["param_group", "param_value"])
    df_stage["stage_index"] = df_stage["stage"].map(STAGE_INDEX)

    return df_kpi, df_stage


def df_subset(df_kpi, group_name):
    """Return KPI subset for a given param_group, sorted by param_value."""
    sub = df_kpi[df_kpi["param_group"] == group_name].copy()
    return sub.sort_values("param_value")


def stage_subset(df_stage, group_name):
    sub = df_stage[df_stage["param_group"] == group_name].copy()
    return sub.sort_values(["param_value", "stage_index"])


def savefig(name):
    path = PLOT_DIR / name
    plt.tight_layout()
    plt.savefig(path, bbox_inches="tight")
    plt.close()
    print(f"Saved {path}")


# ---------------------------------------------------------------------------
# Excess air plots
# ---------------------------------------------------------------------------

def plot_excess_air_boiler_overview(df_ea):
    lam = df_ea["param_value"]
    q_in = df_ea["Q_in total[MW]"]
    q_useful = df_ea["Q_useful[MW]"]
    eta_d = df_ea["eta direct[-]"]
    eta_i = df_ea["eta indirect[-]"]
    t_stack = df_ea["stack temperature[°C]"]
    m_water = df_ea["water flow[kg/s]"]
    dp_total = df_ea["pressure drop total[Pa]"]

    fig, axes = plt.subplots(2, 2, figsize=(6.0, 4.5))

    # Q_in / Q_useful
    ax = axes[0, 0]
    ax.plot(lam, q_in, "o-", label=r"$Q_\mathrm{in}$")
    ax.plot(lam, q_useful, "s-", label=r"$Q_\mathrm{useful}$")
    ax.set_xlabel(r"Excess air ratio $\lambda$ [-]")
    ax.set_ylabel("Heat rate [MW]")
    ax.set_title("Heat input and useful duty")
    ax.legend()

    # Efficiencies
    ax = axes[0, 1]
    ax.plot(lam, eta_d, "o-", label=r"$\eta_\mathrm{direct}$")
    ax.plot(lam, eta_i, "s-", label=r"$\eta_\mathrm{indirect}$")
    ax.set_xlabel(r"Excess air ratio $\lambda$ [-]")
    ax.set_ylabel("Efficiency [-]")
    ax.set_title("Boiler efficiency")
    ax.legend()

    # Stack temperature & water flow
    ax = axes[1, 0]
    ax2 = ax.twinx()
    ax.plot(lam, t_stack, "o-", label=r"$T_\mathrm{stack}$")
    ax2.plot(lam, m_water, "s--", label=r"$\dot m_w$")
    ax.set_xlabel(r"Excess air ratio $\lambda$ [-]")
    ax.set_ylabel(r"Stack temperature [°C]")
    ax2.set_ylabel(r"Water flow [kg/s]")
    ax.set_title("Stack temperature and water flow")

    # Manually combine legends for twinx
    lines, labels = ax.get_legend_handles_labels()
    lines2, labels2 = ax2.get_legend_handles_labels()
    ax.legend(lines + lines2, labels + labels2, loc="best")

    # Pressure drop
    ax = axes[1, 1]
    ax.plot(lam, -dp_total, "o-")
    ax.set_xlabel(r"Excess air ratio $\lambda$ [-]")
    ax.set_ylabel(r"Total gas side $\Delta P$ [Pa]")
    ax.set_title("Total gas side pressure drop")

    fig.suptitle("Excess air sweep – boiler level overview")
    savefig("fig_lambda_boiler_overview.png")


def plot_excess_air_stage_temperatures(df_stage_ea):
    # For each lambda, plot gas- and water-out vs stage index
    lam_values = sorted(df_stage_ea["param_value"].dropna().unique())
    fig, axes = plt.subplots(1, 2, figsize=(7.5, 3.0), sharey=True)

    for lam in lam_values:
        data = df_stage_ea[df_stage_ea["param_value"] == lam].sort_values("stage_index")
        x = data["stage_index"]
        stages = data["stage"]
        axes[0].plot(
            x,
            data["gas out temp[°C]"],
            marker="o",
            label=fr"$\lambda={lam:.1f}$",
        )
        axes[1].plot(
            x,
            data["water out temp[°C]"],
            marker="s",
            label=fr"$\lambda={lam:.1f}$",
        )

    for ax, title, ylabel in [
        (axes[0], "Gas outlet temperature", r"Temperature [°C]"),
        (axes[1], "Water outlet temperature", r"Temperature [°C]"),
    ]:
        ax.set_xticks(range(1, len(STAGE_ORDER) + 1))
        ax.set_xticklabels(STAGE_ORDER)
        ax.set_xlabel("Heat exchanger stage")
        ax.set_ylabel(ylabel)
        ax.set_title(title)

    axes[0].legend(title=r"Excess air $\lambda$", bbox_to_anchor=(1.05, 1.0))
    fig.suptitle("Excess air sweep – stage temperature profiles")
    savefig("fig_lambda_stage_temperatures.png")


def plot_excess_air_stage_duties_ua(df_stage_ea):
    lam_values = sorted(df_stage_ea["param_value"].dropna().unique())

    # Left: Q_conv, Q_rad, Q_total per stage vs lambda
    fig, axes = plt.subplots(1, 2, figsize=(7.0, 3.0))

    # Left panel: Q per stage vs lambda
    ax = axes[0]
    for stage in STAGE_ORDER:
        data = df_stage_ea[df_stage_ea["stage"] == stage].sort_values("param_value")
        ax.plot(
            data["param_value"],
            data["Q total[MW]"],
            marker="o",
            label=stage,
        )
    ax.set_xlabel(r"Excess air ratio $\lambda$ [-]")
    ax.set_ylabel(r"Stage heat duty $Q_\mathrm{total}$ [MW]")
    ax.set_title("Stage total duties vs excess air")
    ax.legend(title="Stage", bbox_to_anchor=(1.04, 1.0))

    # Right panel: UA per stage vs lambda
    ax = axes[1]
    for stage in STAGE_ORDER:
        data = df_stage_ea[df_stage_ea["stage"] == stage].sort_values("param_value")
        ax.plot(
            data["param_value"],
            data["UA[MW/K]"],
            marker="s",
            label=stage,
        )
    ax.set_xlabel(r"Excess air ratio $\lambda$ [-]")
    ax.set_ylabel(r"Global conductance $UA$ [MW/K]")
    ax.set_title("Stage global conductance vs excess air")

    fig.suptitle("Excess air sweep – stage duties and conductance")
    savefig("fig_lambda_stage_duties.png")


def plot_excess_air_stage_pressure_drop(df_stage_ea):
    lam_values = sorted(df_stage_ea["param_value"].dropna().unique())

    fig, ax = plt.subplots(figsize=(6.0, 3.5))

    for stage in STAGE_ORDER:
        data = df_stage_ea[df_stage_ea["stage"] == stage].sort_values("param_value")
        ax.plot(
            data["param_value"],
            -data["pressure drop total[pa]"],
            marker="o",
            label=stage,
        )

    ax.set_xlabel(r"Excess air ratio $\lambda$ [-]")
    ax.set_ylabel(r"Stage total $\Delta P$ [Pa]")
    ax.set_title("Gas side pressure loss per stage – excess air sweep")
    ax.legend(title="Stage", bbox_to_anchor=(1.04, 1.0))
    savefig("fig_lambda_stage_dp.png")


def plot_excess_air_compact(df_ea):
    lam = df_ea["param_value"]

    fig, axes = plt.subplots(2, 2, figsize=(6.0, 4.5))

    axes[0, 0].plot(lam, df_ea["eta direct[-]"], "o-", label="direct")
    axes[0, 0].plot(lam, df_ea["eta indirect[-]"], "s-", label="indirect")
    axes[0, 0].set_ylabel("Efficiency [-]")
    axes[0, 0].set_xlabel(r"$\lambda$ [-]")
    axes[0, 0].set_title("Efficiency")
    axes[0, 0].legend()

    axes[0, 1].plot(lam, df_ea["stack temperature[°C]"], "o-")
    axes[0, 1].set_ylabel(r"$T_\mathrm{stack}$ [°C]")
    axes[0, 1].set_xlabel(r"$\lambda$ [-]")
    axes[0, 1].set_title("Stack temperature")

    axes[1, 0].plot(lam, df_ea["steam capacity[t/h]"], "o-")
    axes[1, 0].set_ylabel("Steam capacity [t/h]")
    axes[1, 0].set_xlabel(r"$\lambda$ [-]")
    axes[1, 0].set_title("Steam capacity")

    axes[1, 1].plot(lam, -df_ea["pressure drop total[Pa]"], "o-")
    axes[1, 1].set_ylabel(r"Total gas $\Delta P$ [Pa]")
    axes[1, 1].set_xlabel(r"$\lambda$ [-]")
    axes[1, 1].set_title("Total gas pressure drop")

    fig.suptitle("Excess air sweep – compact summary")
    savefig("fig_lambda_compact_summary.png")


# ---------------------------------------------------------------------------
# Drum pressure plots
# ---------------------------------------------------------------------------

def plot_pressure_boiler_overview(df_p):
    P_bar = df_p["param_value"]
    q_in = df_p["Q_in total[MW]"]
    q_useful = df_p["Q_useful[MW]"]
    eta_d = df_p["eta direct[-]"]
    eta_i = df_p["eta indirect[-]"]
    m_w = df_p["water flow[kg/s]"]
    steam_cap = df_p["steam capacity[t/h]"]
    t_stack = df_p["stack temperature[°C]"]
    dp_total = df_p["pressure drop total[Pa]"]

    fig, axes = plt.subplots(2, 2, figsize=(6.0, 4.5))

    ax = axes[0, 0]
    ax.plot(P_bar, q_in, "o-", label=r"$Q_\mathrm{in}$")
    ax.plot(P_bar, q_useful, "s-", label=r"$Q_\mathrm{useful}$")
    ax.set_xlabel("Drum pressure [bar]")
    ax.set_ylabel("Heat rate [MW]")
    ax.set_title("Heat input and useful duty")
    ax.legend()

    ax = axes[0, 1]
    ax.plot(P_bar, eta_d, "o-", label="direct")
    ax.plot(P_bar, eta_i, "s-", label="indirect")
    ax.set_xlabel("Drum pressure [bar]")
    ax.set_ylabel("Efficiency [-]")
    ax.set_title("Boiler efficiency")
    ax.legend()

    ax = axes[1, 0]
    ax2 = ax.twinx()
    ax.plot(P_bar, m_w, "o-", label="water flow")
    ax2.plot(P_bar, steam_cap, "s--", label="steam capacity")
    ax.set_xlabel("Drum pressure [bar]")
    ax.set_ylabel(r"Water flow [kg/s]")
    ax2.set_ylabel(r"Steam capacity [t/h]")
    ax.set_title("Water / steam throughput")

    lines, labels = ax.get_legend_handles_labels()
    lines2, labels2 = ax2.get_legend_handles_labels()
    ax.legend(lines + lines2, labels + labels2, loc="best")

    ax = axes[1, 1]
    ax2 = ax.twinx()
    ax.plot(P_bar, t_stack, "o-", label=r"$T_\mathrm{stack}$")
    ax2.plot(P_bar, -dp_total, "s--", label=r"$\Delta P_\mathrm{gas}$")
    ax.set_xlabel("Drum pressure [bar]")
    ax.set_ylabel(r"Stack temperature [°C]")
    ax2.set_ylabel(r"Total gas $\Delta P$ [Pa]")
    ax.set_title("Stack temperature and gas pressure drop")
    lines, labels = ax.get_legend_handles_labels()
    lines2, labels2 = ax2.get_legend_handles_labels()
    ax.legend(lines + lines2, labels + labels2, loc="best")

    fig.suptitle("Drum pressure sweep – boiler level overview")
    savefig("fig_pressure_boiler_overview.png")


def plot_pressure_steam_tradeoff(df_p, df_stage_p):
    P_bar = df_p["param_value"]
    steam_cap = df_p["steam capacity[t/h]"]

    # Specific steam enthalpy: use water_out enthalpy at drum stage (HX_1 or boiling stage).
    # Approximate using HX_1 water-out enthalpy.
    drum_stage = df_stage_p[df_stage_p["stage"] == "HX_1"]
    h_steam = (
        drum_stage.groupby("param_value")["water out enthalpy[kJ/kg]"]
        .mean()
        .reindex(P_bar.values)
        .values
    )

    fig, axes = plt.subplots(1, 2, figsize=(7.0, 3.0))

    axes[0].plot(P_bar, steam_cap, "o-")
    axes[0].set_xlabel("Drum pressure [bar]")
    axes[0].set_ylabel("Steam capacity [t/h]")
    axes[0].set_title("Steam mass flow vs pressure")

    axes[1].plot(P_bar, h_steam, "s-")
    axes[1].set_xlabel("Drum pressure [bar]")
    axes[1].set_ylabel("Steam specific enthalpy [kJ/kg]")
    axes[1].set_title("Steam enthalpy vs pressure")

    fig.suptitle("Drum pressure sweep – steam trade-off")
    savefig("fig_pressure_steam_tradeoff.png")


def plot_pressure_stage_duties(df_stage_p):
    fig, ax = plt.subplots(figsize=(6.0, 3.5))

    for stage in STAGE_ORDER:
        data = df_stage_p[df_stage_p["stage"] == stage].sort_values("param_value")
        ax.plot(
            data["param_value"],
            data["Q total[MW]"],
            marker="o",
            label=stage,
        )

    ax.set_xlabel("Drum pressure [bar]")
    ax.set_ylabel(r"Stage duty $Q_\mathrm{total}$ [MW]")
    ax.set_title("Stage-wise total duty vs drum pressure")
    ax.legend(title="Stage", bbox_to_anchor=(1.04, 1.0))
    savefig("fig_pressure_stage_duties.png")


def plot_pressure_economiser_detail(df_stage_p):
    eco = df_stage_p[df_stage_p["stage"] == "HX_6"].sort_values("param_value")

    P_bar = eco["param_value"]

    fig, ax = plt.subplots(figsize=(6.0, 3.5))

    ax.plot(P_bar, eco["gas in temp[°C]"], "o-", label=r"$T_{g,\mathrm{in}}$")
    ax.plot(P_bar, eco["gas out temp[°C]"], "s-", label=r"$T_{g,\mathrm{out}}$")
    ax.plot(P_bar, eco["water in temp[°C]"], "d-", label=r"$T_{w,\mathrm{in}}$")
    ax.plot(P_bar, eco["water out temp[°C]"], "x-", label=r"$T_{w,\mathrm{out}}$")
    ax.set_xlabel("Drum pressure [bar]")
    ax.set_ylabel("Temperature [°C]")

    ax2 = ax.twinx()
    ax2.plot(P_bar, eco["Q total[MW]"], "k--", label=r"$Q_{\mathrm{HX6}}$")
    ax2.set_ylabel(r"Economiser duty $Q_{\mathrm{HX6}}$ [MW]")

    lines, labels = ax.get_legend_handles_labels()
    lines2, labels2 = ax2.get_legend_handles_labels()
    ax.legend(lines + lines2, labels + labels2, loc="best")

    ax.set_title("Economiser performance vs drum pressure")
    savefig("fig_pressure_economiser.png")


def plot_pressure_compact(df_p, df_stage_p):
    P_bar = df_p["param_value"]

    # Same enthalpy as before
    drum_stage = df_stage_p[df_stage_p["stage"] == "HX_1"]
    h_steam = (
        drum_stage.groupby("param_value")["water out enthalpy[kJ/kg]"]
        .mean()
        .reindex(P_bar.values)
        .values
    )

    fig, axes = plt.subplots(2, 2, figsize=(6.0, 4.5))

    axes[0, 0].plot(P_bar, df_p["eta direct[-]"], "o-")
    axes[0, 0].set_xlabel("Drum pressure [bar]")
    axes[0, 0].set_ylabel("Efficiency [-]")
    axes[0, 0].set_title("Efficiency")

    axes[0, 1].plot(P_bar, df_p["stack temperature[°C]"], "o-")
    axes[0, 1].set_xlabel("Drum pressure [bar]")
    axes[0, 1].set_ylabel(r"$T_\mathrm{stack}$ [°C]")
    axes[0, 1].set_title("Stack temperature")

    axes[1, 0].plot(P_bar, df_p["steam capacity[t/h]"], "o-")
    axes[1, 0].set_xlabel("Drum pressure [bar]")
    axes[1, 0].set_ylabel("Steam capacity [t/h]")
    axes[1, 0].set_title("Steam capacity")

    axes[1, 1].plot(P_bar, h_steam, "o-")
    axes[1, 1].set_xlabel("Drum pressure [bar]")
    axes[1, 1].set_ylabel("Steam enthalpy [kJ/kg]")
    axes[1, 1].set_title("Steam enthalpy")

    fig.suptitle("Drum pressure sweep – compact summary")
    savefig("fig_pressure_compact_summary.png")


# ---------------------------------------------------------------------------
# Fuel mass flow (firing rate) plots
# ---------------------------------------------------------------------------

def plot_fuel_boiler_overview(df_f):
    mdot = df_f["param_value"]
    q_in = df_f["Q_in total[MW]"]
    q_useful = df_f["Q_useful[MW]"]
    eta_d = df_f["eta direct[-]"]
    eta_i = df_f["eta indirect[-]"]
    steam_cap = df_f["steam capacity[t/h]"]
    t_stack = df_f["stack temperature[°C]"]
    dp_total = df_f["pressure drop total[Pa]"]

    fig, axes = plt.subplots(2, 2, figsize=(6.0, 4.5))

    ax = axes[0, 0]
    ax.plot(mdot, q_in, "o-", label=r"$Q_\mathrm{in}$")
    ax.plot(mdot, q_useful, "s-", label=r"$Q_\mathrm{useful}$")
    ax.set_xlabel(r"Fuel mass flow $\dot m_f$ [kg/s]")
    ax.set_ylabel("Heat rate [MW]")
    ax.set_title("Heat input and useful duty")
    ax.legend()

    ax = axes[0, 1]
    ax.plot(mdot, eta_d, "o-", label="direct")
    ax.plot(mdot, eta_i, "s-", label="indirect")
    ax.set_xlabel(r"Fuel mass flow $\dot m_f$ [kg/s]")
    ax.set_ylabel("Efficiency [-]")
    ax.set_title("Boiler efficiency")
    ax.legend()

    ax = axes[1, 0]
    ax.plot(mdot, steam_cap, "o-")
    ax.set_xlabel(r"Fuel mass flow $\dot m_f$ [kg/s]")
    ax.set_ylabel("Steam capacity [t/h]")
    ax.set_title("Steam capacity")

    ax = axes[1, 1]
    ax2 = ax.twinx()
    ax.plot(mdot, t_stack, "o-", label=r"$T_\mathrm{stack}$")
    ax2.plot(mdot, -dp_total, "s--", label=r"$\Delta P_\mathrm{gas}$")
    ax.set_xlabel(r"Fuel mass flow $\dot m_f$ [kg/s]")
    ax.set_ylabel(r"Stack temperature [°C]")
    ax2.set_ylabel(r"Total gas $\Delta P$ [Pa]")
    ax.set_title("Stack temperature and gas pressure drop")
    lines, labels = ax.get_legend_handles_labels()
    lines2, labels2 = ax2.get_legend_handles_labels()
    ax.legend(lines + lines2, labels + labels2, loc="best")

    fig.suptitle("Firing rate sweep – boiler level overview")
    savefig("fig_fuel_boiler_overview.png")


def plot_fuel_linearity(df_f):
    mdot = df_f["param_value"]
    q_in = df_f["Q_in total[MW]"]
    q_useful = df_f["Q_useful[MW]"]
    steam_cap = df_f["steam capacity[t/h]"]

    fig, axes = plt.subplots(1, 2, figsize=(7.0, 3.0))

    # Left: Q_useful vs Q_in with straight line through origin
    ax = axes[0]
    ax.plot(q_in, q_useful, "o", label="model")
    coeff = np.polyfit(q_in, q_useful, 1)
    q_fit = np.linspace(q_in.min(), q_in.max(), 50)
    ax.plot(q_fit, np.polyval(coeff, q_fit), "k--", label=f"linear fit, slope={coeff[0]:.3f}")
    ax.set_xlabel(r"$Q_\mathrm{in}$ [MW]")
    ax.set_ylabel(r"$Q_\mathrm{useful}$ [MW]")
    ax.set_title("Linearity of useful duty")
    ax.legend()

    # Right: steam capacity vs fuel flow
    ax = axes[1]
    ax.plot(mdot, steam_cap, "o", label="model")
    coeff2 = np.polyfit(mdot, steam_cap, 1)
    mdot_fit = np.linspace(mdot.min(), mdot.max(), 50)
    ax.plot(mdot_fit, np.polyval(coeff2, mdot_fit), "k--", label=f"linear fit, slope={coeff2[0]:.1f}")
    ax.set_xlabel(r"Fuel mass flow $\dot m_f$ [kg/s]")
    ax.set_ylabel("Steam capacity [t/h]")
    ax.set_title("Linearity of steam capacity")
    ax.legend()

    fig.suptitle("Firing rate sweep – linearity checks")
    savefig("fig_fuel_linearity.png")


def plot_fuel_stage_duties_dp(df_stage_f):
    fig, axes = plt.subplots(2, 1, figsize=(6.0, 6.0), sharex=True)

    # Stage duties
    ax = axes[0]
    for stage in STAGE_ORDER:
        data = df_stage_f[df_stage_f["stage"] == stage].sort_values("param_value")
        ax.plot(
            data["param_value"],
            data["Q total[MW]"],
            marker="o",
            label=stage,
        )
    ax.set_ylabel(r"Stage duty $Q_\mathrm{total}$ [MW]")
    ax.set_title("Stage total duties vs fuel mass flow")
    ax.legend(title="Stage", bbox_to_anchor=(1.04, 1.0))

    # Stage pressure drops
    ax = axes[1]
    for stage in STAGE_ORDER:
        data = df_stage_f[df_stage_f["stage"] == stage].sort_values("param_value")
        ax.plot(
            data["param_value"],
            -data["pressure drop total[pa]"],
            marker="s",
            label=stage,
        )
    ax.set_xlabel(r"Fuel mass flow $\dot m_f$ [kg/s]")
    ax.set_ylabel(r"Stage total $\Delta P$ [Pa]")
    ax.set_title("Stage pressure drops vs fuel mass flow")

    fig.suptitle("Firing rate sweep – stage duties and pressure drops")
    savefig("fig_fuel_stage_duty_dp.png")


def plot_fuel_stage_temperatures(df_stage_f):
    mdot_values = sorted(df_stage_f["param_value"].dropna().unique())

    fig, axes = plt.subplots(1, 2, figsize=(7.5, 3.0), sharey=True)

    for mdot in mdot_values:
        data = df_stage_f[df_stage_f["param_value"] == mdot].sort_values("stage_index")
        x = data["stage_index"]
        axes[0].plot(
            x,
            data["gas out temp[°C]"],
            marker="o",
            label=fr"$\dot m_f={mdot:.3f}$",
        )
        axes[1].plot(
            x,
            data["water out temp[°C]"],
            marker="s",
            label=fr"$\dot m_f={mdot:.3f}$",
        )

    for ax, title, ylabel in [
        (axes[0], "Gas outlet temperature", r"Temperature [°C]"),
        (axes[1], "Water outlet temperature", r"Temperature [°C]"),
    ]:
        ax.set_xticks(range(1, len(STAGE_ORDER) + 1))
        ax.set_xticklabels(STAGE_ORDER)
        ax.set_xlabel("Heat exchanger stage")
        ax.set_ylabel(ylabel)
        ax.set_title(title)

    axes[0].legend(title=r"Fuel flow $\dot m_f$ [kg/s]", bbox_to_anchor=(1.05, 1.0))
    fig.suptitle("Firing rate sweep – stage temperature profiles")
    savefig("fig_fuel_stage_temperatures.png")


def plot_fuel_compact(df_f):
    mdot = df_f["param_value"]

    fig, axes = plt.subplots(2, 2, figsize=(6.0, 4.5))

    axes[0, 0].plot(mdot, df_f["eta direct[-]"], "o-")
    axes[0, 0].set_xlabel(r"$\dot m_f$ [kg/s]")
    axes[0, 0].set_ylabel("Efficiency [-]")
    axes[0, 0].set_title("Efficiency")

    axes[0, 1].plot(mdot, df_f["stack temperature[°C]"], "o-")
    axes[0, 1].set_xlabel(r"$\dot m_f$ [kg/s]")
    axes[0, 1].set_ylabel(r"$T_\mathrm{stack}$ [°C]")
    axes[0, 1].set_title("Stack temperature")

    axes[1, 0].plot(mdot, df_f["steam capacity[t/h]"], "o-")
    axes[1, 0].set_xlabel(r"$\dot m_f$ [kg/s]")
    axes[1, 0].set_ylabel("Steam capacity [t/h]")
    axes[1, 0].set_title("Steam capacity")

    axes[1, 1].plot(mdot, -df_f["pressure drop total[Pa]"], "o-")
    axes[1, 1].set_xlabel(r"$\dot m_f$ [kg/s]")
    axes[1, 1].set_ylabel(r"Total gas $\Delta P$ [Pa]")
    axes[1, 1].set_title("Total gas pressure drop")

    fig.suptitle("Firing rate sweep – compact summary")
    savefig("fig_fuel_compact_summary.png")


# ---------------------------------------------------------------------------
# Extra overall summary plot (all runs)
# ---------------------------------------------------------------------------

def plot_global_eta_vs_stack(df_kpi):
    fig, ax = plt.subplots(figsize=(5.0, 3.5))

    groups = df_kpi["param_group"].fillna("control").unique()
    markers = {
        "control": "o",
        "excess_air": "s",
        "water_pressure": "d",
        "fuel_flow": "^",
    }

    for g in groups:
        sub = df_kpi[df_kpi["param_group"].fillna("control") == g]
        ax.scatter(
            sub["stack temperature[°C]"],
            sub["eta indirect[-]"],
            marker=markers.get(g, "o"),
            label=g,
        )

    ax.set_xlabel(r"Stack temperature [°C]")
    ax.set_ylabel(r"Indirect efficiency $\eta_\mathrm{indirect}$ [-]")
    ax.set_title("Global relation of efficiency and stack temperature")
    ax.legend(title="Param group")
    savefig("fig_global_eta_vs_stack.png")


# ---------------------------------------------------------------------------
# Main
# ---------------------------------------------------------------------------

def main():
    df_kpi, df_stage = load_data()

    # Excess air
    df_ea = df_subset(df_kpi, "excess_air")
    df_stage_ea = stage_subset(df_stage, "excess_air")
    if not df_ea.empty:
        plot_excess_air_boiler_overview(df_ea)
        plot_excess_air_stage_temperatures(df_stage_ea)
        plot_excess_air_stage_duties_ua(df_stage_ea)
        plot_excess_air_stage_pressure_drop(df_stage_ea)
        plot_excess_air_compact(df_ea)

    # Drum pressure
    df_p = df_subset(df_kpi, "water_pressure")
    df_stage_p = stage_subset(df_stage, "water_pressure")
    if not df_p.empty:
        plot_pressure_boiler_overview(df_p)
        plot_pressure_steam_tradeoff(df_p, df_stage_p)
        plot_pressure_stage_duties(df_stage_p)
        plot_pressure_economiser_detail(df_stage_p)
        plot_pressure_compact(df_p, df_stage_p)

    # Fuel mass flow
    df_f = df_subset(df_kpi, "fuel_flow")
    df_stage_f = stage_subset(df_stage, "fuel_flow")
    if not df_f.empty:
        plot_fuel_boiler_overview(df_f)
        plot_fuel_linearity(df_f)
        plot_fuel_stage_duties_dp(df_stage_f)
        plot_fuel_stage_temperatures(df_stage_f)
        plot_fuel_compact(df_f)

    # Extra global summary
    plot_global_eta_vs_stack(df_kpi)


if __name__ == "__main__":
    main()
